...
// for demo only
#define PRODUCT_KEY      "a1HzFxnfjJj"
#define PRODUCT_SECRET   "dgKE2EHK4wn4KKWi"
#define DEVICE_NAME      "8YprsFxubS80d4NHGUY2"
#define DEVICE_SECRET    "82c11106ed0b2a9cb3abcc525fd826b5"
...
static uint8_t g_lightSensorStatus = 0;

extern gpio_dev_t lightsensor;
...
void hal_lightsensor_state_post_property(void)
{
    int res = 0;
  
    char property_payload[30] = {0};

    HAL_Snprintf(property_payload, sizeof(property_payload), "{\"lightSensor\": %d}", g_lightSensorStatus);
...
void linkkit_lightsensor_check(void)
{
    uint8_t mStatus = 0;
    
    //读取光敏状态
    if(!hal_gpio_input_get(&lightsensor,&mStatus))
    {
        if(mStatus != g_lightSensorStatus)
        {
            //更新状态
            g_lightSensorStatus = mStatus;
            //上报状态
            hal_lightsensor_state_post_property();
        }
    }
}
...
int linkkit_main(void *paras)
{
    ...
    //同步状态
    hal_gpio_input_get(&lightsensor,&g_lightSensorStatus);

    while (1) {
        IOT_Linkkit_Yield(EXAMPLE_YIELD_TIMEOUT_MS);
        //10s 定时检测
        if ((cnt % 50) == 0) {
            linkkit_lightsensor_check();
            cnt = 0;
        }
        cnt++;
    }
