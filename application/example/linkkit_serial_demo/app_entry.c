...
static void handle_led_switch_cmd(char *pwbuf, int blen, int argc, char **argv)
{
    if(argc == 1)                      
    {
        LOG("CMD ERROR");
...
static struct cli_command led_switch_cmd = {
    .name     = "led_switch",
    .help     = "led_switch [on/off]turn on/off led",
    .function = handle_led_switch_cmd
};
...
int application_start(int argc, char **argv)
{
	...
	aos_cli_register_command(&led_switch_cmd);
#ifdef AWSS_SUPPORT_DEV_AP
