...
static char g_buzzer_status = 0;

extern gpio_dev_t buzzer;

int linkkit_keys_handle(void);
...
static int user_property_set_event_handler(const int devid, const char *request, const int request_len)
{
    ...
    sw = cJSON_GetObjectItem(rJson,"buzzerSwitch");
	if(sw)
	{
		g_buzzer_status = sw->valueint;
		EXAMPLE_TRACE("relayState=%d", g_buzzer_status);
		if(g_buzzer_status){
            linkkit_start_alarm();
        }
	}

    hal_buzzer_state_post_property();
...
void hal_buzzer_state_post_property(void)
{
    int res = 0;

    char property_payload[30] = {0};
    HAL_Snprintf(property_payload, sizeof(property_payload), "{\"buzzerSwitch\": %d}", g_buzzer_status);
...
static void hal_alarm_poll_func(void *arg)
{
    //翻转
    hal_gpio_output_toggle(&buzzer);
    if(g_buzzer_status){
        //500ms 间隔响一次
        aos_post_delayed_action(500, hal_alarm_poll_func, NULL);
    }else{
        //关闭蜂鸣器
        hal_gpio_output_low(&buzzer);
    }
}

void linkkit_start_alarm(void)
{
     aos_schedule_call(hal_alarm_poll_func, NULL);
}
