...
// for demo only
#define PRODUCT_KEY      "a1Xmk8IPiox"
#define PRODUCT_SECRET   "p4v1T5Su1ZUXMJu7"
#define DEVICE_NAME      "Dv0Z8cbUBZgdcjaUpxJT"
#define DEVICE_SECRET    "AIp39FN6Xy28nKm6QsDaqbvgBg5mCgJx"
...
static int user_property_set_event_handler(const int devid, const char *request, const int request_len)
{
    ...
    sw = cJSON_GetObjectItem(rJson,"LockEnabled");
	if(sw)
	{
		maglockState = sw->valueint;
		EXAMPLE_TRACE("maglockState=%d", maglockState);
		if(maglockState)
            hal_maglock_on();
        else
            hal_maglock_off();
    ...
    hal_maglock_state_post_property();
...
void hal_maglock_state_post_property(void)
{
    int res = 0;

    char property_payload[30] = {0};
    HAL_Snprintf(property_payload, sizeof(property_payload), "{\"LockEnabled\": %d}", g_maglock_state);
...
{
    g_maglock_state = 1;
    hal_gpio_output_high(&maglock);
}

int hal_maglock_off(void)
{
    g_maglock_state = 0;
    hal_gpio_output_low(&maglock);
}
