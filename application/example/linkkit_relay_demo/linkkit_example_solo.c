...
static char g_relay_state = 0;

extern gpio_dev_t relay;
int hal_relay_off(void);
int hal_relay_on(void);
...
void hal_relay_state_post_property(void)
{
    int res = 0;

    char property_payload[30] = {0};
    HAL_Snprintf(property_payload, sizeof(property_payload), "{\"PowerSwitch_1\": %d}", g_relay_state);
...
{
    g_relay_state = 1;
    hal_gpio_output_high(&relay);
}

int hal_relay_off(void)
{
    g_relay_state = 0;
    hal_gpio_output_low(&relay);
}
