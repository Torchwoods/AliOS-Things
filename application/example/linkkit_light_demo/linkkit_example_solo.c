...
static int user_property_set_event_handler(const int devid, const char *request, const int request_len)
{
    ...
    sw = cJSON_GetObjectItem(rJson,"LightSwitch");
	if(sw)
	{
		ledstate = sw->valueint;
		EXAMPLE_TRACE("ledstate=%d", ledstate);
		if(ledstate)
            hal_led_on();
        else
            hal_led_off();
...
void hal_led_state_post_property(void)
{
    int res = 0;

    char property_payload[30] = {0};
    HAL_Snprintf(property_payload, sizeof(property_payload), "{\"LightSwitch\": %d}", g_led_state);
...
{
    g_led_state = 1;
    hal_gpio_output_high(&leds[0]);
}

int hal_led_off(void)
{
    g_led_state = 0;
    hal_gpio_output_low(&leds[0]);
}
